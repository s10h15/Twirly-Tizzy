...
export function getTinyMushroom(vertexes, images) {
    const tinyMushroom = {
        vertexes: vertexes,
        name: "tinyMushroom",
        ...
        textureWidth: 0,
        textureHeight: 0,
        imageToUseInTexture: images.tinyMushroom || null,
        ...
        updateStep(player, world, inventory) {
            if (this.isCollidesCircle(player.x, player.y, player.radius)) {
                player.radius = player.tinyRadius;
                player.speed = player.tinySpeed;
                this.isShouldDisappear = true;
                playEffect("stretch1");
            }
        }
    }
    tinyMushroom.generateTexture();
    return tinyMushroom;
}