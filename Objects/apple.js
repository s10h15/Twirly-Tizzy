...
export function getApple(vertexes, images) {
    const apple = {
        vertexes: vertexes,
        name: "apple",
        ...
        textureWidth: 0,
        textureHeight: 0,
        imageToUseInTexture: images.apple ?? null,
        numToGive: 1,
        ...
        use(player, gameStepsCount, world, images) {
            if (player.isSpedUp || world.objects.fires[0].remainingStrength <= 0) {
                return false;
            } else {
                player.speedUp(gameStepsCount);
                return true;
            }
        }
    }
    apple.updateCenter();
    apple.generateTexture();
    return apple;
}